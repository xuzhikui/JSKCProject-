...
//
    ...
    img.image = [UIImage imageNamed:@"已有司机"];
    [back addSubview:img];
    ...
    [UILabel initWithDFLable:CGRectMake(0,120,back.frame.size.width, 30) :[UIFont systemFontOfSize:20*Width1] :COLOR2(51) :@"司机已存在,是否直接添加" :back :1];
...
-(void)resAction{
    ...
    [AFN_DF POST:@"/system/driver/driectAddDriver" Parameters:@{@"phone":self.phone} success:^(NSDictionary *responseObject) {
        
        
        
        for (UIViewController *vc in [[AFN_DF topViewController].navigationController viewControllers]) {
            
            if ([vc isKindOfClass:[DriverListController class]] || [vc isKindOfClass:[SourceGoodController class]]) {
