...
@interface EndUpPhoneController ()
{
    UIButton *codeBut;
}
...
@implementation EndUpPhoneController
...
-(void)setUI{
    ...
    UIView *phoneVC = [[UIView alloc]initWithFrame:CGRectMake(0, NavaBarHeight + 40, Width, 50)];
      ...
      self.userTF = [[UITextField alloc]initWithFrame:CGRectMake(100, 12.5, Width - 200, 25)];
        _userTF.keyboardType = UIKeyboardTypePhonePad;
          self.userTF.placeholder = @"请输入新手机号";
          self.userTF.font = [UIFont systemFontOfSize:15*Width1];
          [phoneVC addSubview:self.userTF];
      
      
              codeBut = [UIButton initWithFrame:CGRectMake(Width - 100, 12.5, 80, 25) :@"发送验证码" :15*Width1];
      ...
      _codeTF = [[UITextField alloc]initWithFrame:CGRectMake(100, 12.5, Width - 200, 25)];
      ...
      UIButton *endBut = [UIButton initWithFrame:CGRectMake(20, NavaBarHeight + 180, Width - 40, 50) :@"确认绑定" :15];
...
-(void)endActrion{
    ...
    if ([self.userTF.text isEqualToString:@""]) {
           ...
           return [self.view addSubview:[Toast makeText:@"请输入新手机号"]];
           
       }
    
    WeakSelf
    [AFN_DF POST:@"/system/account/updatePhone" Parameters:@{@"code":self.codeTF.text, @"phone":self.userTF.text,@"type":@"2"} success:^(NSDictionary *responseObject) {
...
-(void)codeAction{
            ...
            [AFN_DF POST:@"/system/account/getSMS" Parameters:@{@"phone":self.userTF.text,@"type":@"2"} success:^(NSDictionary *responseObject) {
