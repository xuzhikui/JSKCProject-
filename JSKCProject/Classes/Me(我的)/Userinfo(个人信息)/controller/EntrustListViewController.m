...
@implementation EntrustListTableViewCell
...
- (void)setInfo:(id)info{
    _info = info;
    NSMutableAttributedString *attrubtuedStr = [[NSMutableAttributedString alloc] initWithString:[NSString stringWithFormat:@"%@  |  %@",self.info[@"name"],self.info[@"phone"]]];
    [attrubtuedStr setAttributes:@{NSFontAttributeName:[UIFont systemFontOfSize:15],NSForegroundColorAttributeName:COLOR2(102)} range:NSMakeRange(0, attrubtuedStr.length)];
    [attrubtuedStr addAttributes:@{NSForegroundColorAttributeName:COLOR2(51)} range:NSMakeRange(0, [self.info[@"name"] length])];
    [attrubtuedStr addAttributes:@{NSFontAttributeName:[UIFont systemFontOfSize:13]} range:[attrubtuedStr.string rangeOfString:self.info[@"phone"]]];
    self.contentLabel.attributedText = attrubtuedStr;
}

@end

@interface EntrustListViewController ()<UITableViewDelegate,UITableViewDataSource>
...
- (UITableView *)contentTableView{
    if (_contentTableView == nil) {
        ...
        [_contentTableView registerClass:EntrustListTableViewCell.class forCellReuseIdentifier:NSStringFromClass(EntrustListTableViewCell.class)];
...
- (UIButton *)addButton{
    if (_addButton == nil) {
        ...
        [_addButton setTitle:@"新增委托收款人" forState:UIControlStateNormal];
...
- (void)addEntrustFunc{
    [self.navigationController pushViewController:AddEntrustViewController.new animated:YES];
}
...
- (NSArray<UITableViewRowAction *> *)tableView:(UITableView *)tableView editActionsForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    UITableViewRowAction *deleteAction = [UITableViewRowAction rowActionWithStyle:UITableViewRowActionStyleDefault title:@"取消委托" handler:^(UITableViewRowAction *action, NSIndexPath *indexPath) {
        if (indexPath.row >= 0 && indexPath.row < weakSelf.list.count) {
            ...
            StrongSelf
            [AFN_DF POST:@"/system/entrust/untrust" Parameters:@{@"truckerId":info[@"truckerId"]} success:^(NSDictionary *responseObject) {
...
- (void)postData{
    WeakSelf
    [AFN_DF POST:@"/system/entrust/get" Parameters:nil success:^(NSDictionary *responseObject) {
