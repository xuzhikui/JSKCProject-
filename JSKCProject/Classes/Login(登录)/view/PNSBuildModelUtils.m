...
+ (TXCustomModel *)buildFullScreenPortraitModelWithButton1Title:(NSString *)button1Title
    ...
    model.navBackImage = [UIImage imageNamed:@"钱包back"];
    model.logoImage = [UIImage imageNamed:@"驾驶证认证"];
...
+ (NSDictionary *)deleteEmpty:(NSDictionary *)dic
{
    ...
    for (id obj in mdic.allKeys)
    {
        ...
        {
            NSDictionary *changeDic = [PNSBuildModelUtils deleteEmpty:value];
            [dicSet setObject:changeDic forKey:obj];
        }
        else if ([value isKindOfClass:[NSArray class]])
        {
            NSArray *changeArr = [PNSBuildModelUtils deleteEmptyArr:value];
...
+ (NSArray *)deleteEmptyArr:(NSArray *)arr
{
    ...
    for (id obj in marr)
    {
        ...
        {
            NSDictionary *changeDic = [PNSBuildModelUtils deleteEmpty:obj];
            NSInteger index = [marr indexOfObject:obj];
            [dicSet setObject:changeDic forKey:@(index)];
        }
        else if ([obj isKindOfClass:[NSArray class]])
        {
            NSArray *changeArr = [PNSBuildModelUtils deleteEmptyArr:obj];
            NSInteger index = [marr indexOfObject:obj];
