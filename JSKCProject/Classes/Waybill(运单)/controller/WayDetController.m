...
@interface WayDetController ()<UITableViewDelegate,UITableViewDataSource,MAMapViewDelegate,AMapNaviDriveManagerDelegate,   AMapNaviDriveViewDelegate,AMapNaviCompositeManagerDelegate>
{
     UIView *backVC;
    SelectCarView *carVC;
      NSTimer *timer;
...
@implementation WayDetController
...
-(void)setUI{
   ...
   lookWayBut.contentHorizontalAlignment = 0;
      ...
      UIView *addressVC = [[UIView alloc]initWithFrame:CGRectMake(10, 50, Width - 20 ,210)];
        ...
        self.addressLab = [UILabel initWithDFLable:CGRectMake(45, 20.5, Width - 70, 20) :[UIFont systemFontOfSize:16*Width1] :[UIColor blackColor] :@"" :addressVC :0];
    ...
    self.starttoImg.userInteractionEnabled = YES;
        ...
        self.unaddressLab =  [UILabel initWithDFLable:CGRectMake(45, 83.5, Width - 70, 20) :[UIFont systemFontOfSize:16*Width1] :[UIColor blackColor] :@"" :addressVC :0];
    ...
    if (self.code == 0) {
          ...
          _mainView.contentSize = CGSizeMake(0, 631);
        UIView *tabbarVC = [[UIView alloc]initWithFrame:CGRectMake(0, Height - 95, Width, 95)];
        tabbarVC.backgroundColor = [UIColor whiteColor];
        [self.view addSubview:tabbarVC];
        [UILabel initWithDFLable:CGRectMake(15, 10, 65, 20) :[UIFont systemFontOfSize:13] :COLOR2(119) :@"运单价格：" :tabbarVC :0];
        self.carLab = [UILabel initWithDFLable:CGRectMake(82.5, 5, 150, 30) :[UIFont systemFontOfSize:18] :COLOR(245, 12, 12) :@"" :tabbarVC :0];
        
        
//
//        NSDictionary *attrs1 = @{NSFontAttributeName:[UIFont systemFontOfSize:14*Width1]};
//           CGFloat widths = [@"更换车辆" boundingRectWithSize:CGSizeZero options:NSStringDrawingUsesLineFragmentOrigin attributes:attrs1 context:nil].size.width;
//
//
//
//
//           UIButton *lookWayBut =[UIButton initWithFrame:CGRectMake(Width - 140, 5, 120, 30) :@"更换车辆":14*Width1];
//           [lookWayBut setImage:[UIImage imageNamed:@"向上"] forState:0];
//           [lookWayBut setTitleColor:[UIColor redColor] forState:0];
//            [lookWayBut addTarget:self action:@selector(seleCarAction) forControlEvents:(UIControlEventTouchUpInside)];
//           [lookWayBut setImageEdgeInsets:UIEdgeInsetsMake(0, widths/2 + lookWayBut.frame.size.width/2 + 5, 0, 0)];
//          lookWayBut.contentHorizontalAlignment = 0;
//              [tabbarVC addSubview:lookWayBut];
        

        UIButton *canlbut = [UIButton initWithFrame:CGRectMake(20,40, Width/2 - 30, 40) :@"取消运单" :16*Width1];
           canlbut.layer.cornerRadius = 4;
           [canlbut setTitleColor:[UIColor redColor] forState:0];
          canlbut.layer.cornerRadius = 4;
          canlbut.layer.borderWidth  = 1;
           [canlbut addTarget:self action:@selector(canlActrion) forControlEvents:(UIControlEventTouchUpInside)];
           canlbut.layer.borderColor = [[UIColor redColor]CGColor];
           [tabbarVC addSubview:canlbut];
           
           
           UIButton *endbut = [UIButton initWithFrame:CGRectMake(Width/2 + (10), 40,Width/2 - 30, 40) :@"确认发货" :16*Width1];
        ...
        if ([state isEqualToString:@"4"]) {
            _mainView.contentSize = CGSizeMake(0, 640);
                  UIView *tabbarVC = [[UIView alloc]initWithFrame:CGRectMake(0, Height - 95, Width, 95)];
                        tabbarVC.backgroundColor = [UIColor whiteColor];
                        [self.view addSubview:tabbarVC];
                        
            self.carLab = [UILabel initWithDFLable:CGRectMake(20, 10, 200, 20) :[UIFont systemFontOfSize:15*Width1] :COLOR2(51) :@"" :tabbarVC :0];
                           
                UIButton *endbut = [UIButton initWithFrame:CGRectMake(20, 40,Width-40, 40) :@"确认到达" :16*Width1];
                [endbut setTitleColor:[UIColor whiteColor] forState:0];
                endbut.layer.cornerRadius = 4;
                endbut.layer.cornerRadius = 4;
                endbut.backgroundColor = COLOR(227, 23, 23);
                [endbut addTarget:self action:@selector(endinButAction) forControlEvents:(UIControlEventTouchUpInside)];
                [tabbarVC addSubview:endbut];
        }else{
        ...
        _mainView.contentSize = CGSizeMake(0, 750);
        
        UIView *tabbarVC = [[UIView alloc]initWithFrame:CGRectMake(0, Height - 60, Width, 60)];
              tabbarVC.backgroundColor = [UIColor whiteColor];
              [self.view addSubview:tabbarVC];
        
                UIButton *endbut = [UIButton initWithFrame:CGRectMake(20, 10,Width-40, 40) :@"删除订单" :16*Width1];
                       ...
                       endbut.backgroundColor =[UIColor whiteColor];
                    [endbut addTarget:self action:@selector(deleButAction) forControlEvents:(UIControlEventTouchUpInside)];
                    [tabbarVC addSubview:endbut];
}
...
-(void)endfwinfo{
    ...
    _mainView.contentSize = CGSizeMake(0, 1330);
...
//                imgVC2.backgroundColor = [UIColor yellowColor];
    ...
    [imgVC2 sd_setImageWithURL:[NSURL URLWithString:self.data[@"unloadPic"]] placeholderImage:[UIImage imageNamed:@"照片1"]];
 
    
    UIView *backVC2  = [[UIView alloc]initWithFrame:CGRectMake(10, 1145, Width - 20,170)];
          backVC2.backgroundColor = [UIColor whiteColor];
            backVC2.layer.cornerRadius = 4;
          [self.mainView addSubview:backVC2];
...
//
    ...
    wayVC.bolock = ^(NSDictionary * _Nonnull dic) {
        ...
        [self calnWayOrder:dic isCancel:YES];
...
-(void)calnWayOrder:(NSDictionary *)dic isCancel:(BOOL)isCancel{
...
//  [self.navigationController popToRootViewControllerAnimated:YES];
    ...
    endVC.block = ^(NSDictionary * _Nonnull dic) {
           ...
           for (UIViewController *vc in [self.navigationController viewControllers]) {
               ...
               if ([vc isKindOfClass:[WaybilController class]]) {
...
-(void)postDet{
    ...
    [AFN_DF POST:@"/tsmanage/waybill/getbyid" Parameters:dic success:^(NSDictionary *responseObject) {
...
-(void)dataSoursAction{
    ...
    self.carLab.text = [NSString stringWithFormat:@"%@%@",self.data[@"freight"],self.data[@"freightType"]];
...
-(void)seleCarAction{
    ...
    UITapGestureRecognizer *TapGR = [[UITapGestureRecognizer alloc]initWithTarget:self action:@selector(onTapGR)];
           ...
           [self.view addSubview:backVC];
            ...
            __weak WayDetController *weakSelf = self;
